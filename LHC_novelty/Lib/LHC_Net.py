...
class LHC_Module_Spatial(tf.keras.layers.Layer):
    def __init__(self, pool_size, head_emb_dim, num_heads, num_channels, resolution, kernel_size, norm_c, name):
        ...
        self.final_dense = tf.keras.layers.Dense(units = resolution * resolution , activation = 'linear')
    ...
    def VectScaledDotProdAttention(self, query, key, value):
        ...
        # print("Reduce means norm score : " ,norm_scores.shape)
    ...
    def call(self, x):
        ...
        head_res_dim = self.resolution

        # print("Batch size : " , batch_size)
        # print("Num channels : " , num_channels)
        # print("Resolution : ", resolution )
        # print("head_res_dim : " ,head_res_dim)
        # print("pool size : ", self.pool_size)
        # print("kernek size : ", self.kernel_size )
        # print("norm_c : ", self.norm_c )
        ...
        query = tf.transpose(query, perm=[0, 2, 1])  # (batch_zsize, num_channels, resolution^2)
        # print("Transpose : " , query.shape)
        q = [None] * self.num_heads
        for i in range(self.num_heads):
            q[i] = self.Wqk[i](query)  # (batch_size, num_channels, head_emb_dim)
...
\
        ...
        # print("Attentions transpose : ", attentions.shape)
        attention = tf.reshape(attentions, shape=(batch_size, num_channels, self.num_heads * resolution * resolution))  # (batch_size, num_channels, resolution^2)
        # print("Attentions reshape : ", attention.shape)
        attention = self.final_dense(attention) #(batch_size, num_channels, resolution^2)
        # print("Attention final dense : " , attention.shape)
        attention = tf.transpose(attention, perm=[0, 2, 1])  # (batch_size, resolution^2, num_channels)
        # print("Attentions transpose: ", attention.shape)
...
#Convolution + batch_normalization + max_pooling -> 224 * 224 -> 56 * 56
class LHCResBlockSmall0(tf.keras.Model, ABC):
    def __init__(self, input_shape, resolution, att_num_channel, num_heads, att_embed_dim, att_kernel_size, pool_size, norm_c):
        ...
        self.LHC_Module = LHC_Module_Spatial(pool_size=pool_size,
                                     resolution=resolution,
                                     num_channels=att_num_channel,
                                     num_heads=4,
                                     head_emb_dim=att_embed_dim,
...
class LHC_ResNet34(tf.keras.Model, ABC):
    def __init__(self, input_shape, num_classes, att_params):
        ...
        self.Input = tf.keras.layers.InputLayer(input_shape=input_shape)
        # self.input_spatial_attention = LHC_Module_Spatial(pool_size = att_params['pool_size'][0],head_emb_dim = att_params['att_embed_dim'][0],num_heads = 4, num_channels = 3, resolution = 224, 
        #                                      kernel_size =att_params['kernel_size'][0],norm_c = att_params['norm_c'][0],name = 'Input spatial attention unit' )
    ...
    def call(self, x):
        x = self.Input(x)
        # print("\nInput shape : " ,x.shape)
        # x - self.input_spatial_attention(x)
        # # print("\nInput spatial Attention shape : " , x.shape)
        x = self.conv1(x)
        # print("\nConv1 shape : " ,x.shape)

        x = self.conv2_1(x)
        # print("\nConv2_1 shape : " ,x.shape)
        x = self.conv2_2(x)
        # print("\nConv2_2 shape : " ,x.shape)
        x = self.conv2_3(x)
        # print("\nConv2_3 shape : " ,x.shape)
        x = self.conv3_1(x)
        x = self.conv3_2(x)
        x = self.conv3_3(x)
        x = self.conv3_4(x)
